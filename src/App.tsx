...
export default function App() {
  ...
  // Due-today and overdue task notifications
  useEffect(() => {
  ...
  const toggleTimer = () => setIsActive(!isActive);  const resetTimer = useCallback(() => { setIsActive(false); setTimeLeft(modes[mode].duration); }, [mode, modes]);
  ...
  const heatmapData = useMemo(() => {
    // Build a map of completed tasks/subtasks by their dueDate (fallback to createdAt date)
  ...
  const handleDashboardAddTask = (e: React.FormEvent) => {
    ...
    setTasks([newTask, ...tasks]);
    // Notify immediately if due today or overdue
    if (dashboardDueDate) {
      const today = new Date().toISOString().split('T')[0];
      if (dashboardDueDate === today) {
        notify(`due-today-${newTask.id}-${today}`, 'Task due today', `"${newTask.text}" is due today.`);
      } else if (dashboardDueDate < today) {
        notify(`overdue-${newTask.id}-${today}`, 'Overdue task', `"${newTask.text}" is overdue.`);
      }
    }
  ...
  const handleCalendarAddTask = (date: string) => {
    ...
    const newTask: Task = {
      id: crypto.randomUUID(), text: calendarNewTaskText.trim(), completed: false,
      priority: 'medium', createdAt: Date.now(), dueDate: date, subTasks: [],
    };
    setTasks(prev => [newTask, ...prev]);
    setCalendarNewTaskText('');
    ...
    <div className="min-h-screen bg-[#050505] text-white font-sans selection:bg-white/20">
      ...
      <div className="max-w-6xl mx-auto px-6 py-8 md:py-12">
        <header className="flex justify-between items-center mb-10">
          ...
          <button onClick={() => setShowSettings(true)} className="p-2 hover:bg-white/5 rounded-xl transition-colors text-white/40 relative">
            <Settings className="w-5 h-5" />
            {notifPermission === 'granted' && <span className="absolute top-1 right-1 w-2 h-2 bg-emerald-400 rounded-full" title="Notifications enabled" />}
          </button>
        </header>
                ...
                <form onSubmit={handleDashboardAddTask} className="space-y-4">
                  ...
                  <div className="flex gap-2 items-center">
                    ...
                    <div className="relative">
                      <button
                        type="button"
                        onClick={() => setShowDashboardDatePicker(v => !v)}
                      ...
                      {showDashboardDatePicker && (
                        <div className="absolute bottom-full mb-2 right-0 z-20 p-3 bg-[#1a1a1a] border border-white/10 rounded-xl shadow-xl">
                          <input
                            ...
                            onChange={e => { setDashboardDueDate(e.target.value); setShowDashboardDatePicker(false); }}
                            className="bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-white"
                            autoFocus
                          />
                          {dashboardDueDate && (
                            <button type="button" onClick={() => { setDashboardDueDate(''); setShowDashboardDatePicker(false); }} className="mt-2 w-full text-[10px] text-white/40 hover:text-red-400 text-center">Clear date</button>
                          )}
                        </div>
                      )}
                    </div>
                        ...
                        <div className="flex items-center gap-4 p-4 floating-card">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <input
                              ...
                              className="mt-1 bg-transparent border-0 text-[10px] text-white/30 hover:text-white/60 cursor-pointer p-0 w-auto"
                              title="Set due date"
                            />
                          </div>
                      ...
                      return (<div key={i} className={cn("p-2 rounded-xl transition-all min-h-[80px]", !day.isCurrentMonth && "opacity-30", isToday && "bg-white/10 ring-1 ring-white/30", calendarAddingDate === day.date ? "bg-white/10" : "hover:bg-white/5 cursor-pointer")}
                        ...
                        {day.tasks.slice(0, 2).map(t => (
                          <div key={t.id} onClick={e => e.stopPropagation()}>
                            <div
                              className={cn("text-[10px] px-1.5 py-0.5 rounded truncate cursor-pointer mb-0.5 flex items-center gap-1", getTaskStatusColor(t, getToday()), t.completed && "line-through opacity-60")}
                              onClick={() => toggleTask(t.id)}
                            >
                              <span className="flex-1 truncate">{t.text}</span>
                              {t.subTasks.length > 0 && (
                                <button onClick={e => { e.stopPropagation(); toggleExpanded(t.id); }} className="shrink-0 hover:opacity-80">
                                  {expandedTasks.has(t.id) ? <ChevronUp className="w-2.5 h-2.5" /> : <ChevronDown className="w-2.5 h-2.5" />}
                                </button>
                              )}
                            </div>
                            {t.subTasks.length > 0 && expandedTasks.has(t.id) && (
                              <div className="ml-1 mb-1 space-y-0.5">
                                {t.subTasks.map(st => (
                                  <div key={st.id} onClick={e => { e.stopPropagation(); toggleSubTask(t.id, st.id); }}
                                    className={cn("text-[9px] px-1.5 py-0.5 rounded cursor-pointer flex items-center gap-1", st.completed ? "text-emerald-400/60 line-through" : "text-white/50 hover:text-white/80")}>
                                    {st.completed ? <CheckCircle2 className="w-2 h-2 shrink-0" /> : <Circle className="w-2 h-2 shrink-0" />}
                        ...
                        {calendarAddingDate === day.date && (
                          <div onClick={e => e.stopPropagation()} className="mt-1">
                            <input
                              type="text"
                              value={calendarNewTaskText}
                              onChange={e => setCalendarNewTaskText(e.target.value)}
                              onKeyDown={e => { if (e.key === 'Enter') handleCalendarAddTask(day.date); if (e.key === 'Escape') { setCalendarAddingDate(null); setCalendarNewTaskText(''); } }}
                              placeholder="Task name..."
                              className="w-full bg-white/10 border border-white/20 rounded px-1.5 py-1 text-[10px] text-white"
                              autoFocus
                ...
                {calendarView === 'week' && (
                  <div className="grid grid-cols-7 gap-2 flex-1">
                    {Array.from({ length: 7 }, (_, i) => {
                      ...
                      return (<div key={i} className={cn("p-2 rounded-xl min-h-[150px] hover:bg-white/5 transition-colors cursor-pointer", isToday && "bg-white/10", calendarAddingDate === dateStr && "bg-white/10")}
                        onClick={() => { if (calendarAddingDate !== dateStr) setCalendarAddingDate(dateStr); }}>
                        ...
                        {dayTasks.slice(0, 3).map(t => (
                          <div key={t.id} onClick={e => e.stopPropagation()}>
                            <div onClick={() => toggleTask(t.id)} className={cn("text-[10px] px-1.5 py-1 rounded truncate cursor-pointer mb-0.5 flex items-center gap-1", getTaskStatusColor(t, getToday()), t.completed && "line-through opacity-60", "hover:opacity-80")}>
                              <span className="flex-1 truncate">{t.text}</span>
                              {t.subTasks.length > 0 && (
                                <button onClick={e => { e.stopPropagation(); toggleExpanded(t.id); }} className="shrink-0">
                                  {expandedTasks.has(t.id) ? <ChevronUp className="w-2.5 h-2.5" /> : <ChevronDown className="w-2.5 h-2.5" />}
                                </button>
                              )}
                            </div>
                        ...
                        {calendarAddingDate === dateStr && (
                          <div onClick={e => e.stopPropagation()} className="mt-1">
                            <input
                              ...
                              value={calendarNewTaskText}
                              onChange={e => setCalendarNewTaskText(e.target.value)}
                              onKeyDown={e => { if (e.key === 'Enter') handleCalendarAddTask(dateStr); if (e.key === 'Escape') { setCalendarAddingDate(null); setCalendarNewTaskText(''); } }}
                              placeholder="Task name..."
                              className="w-full bg-white/10 border border-white/20 rounded px-1.5 py-1 text-[10px] text-white"
                              autoFocus
                              onClick={e => e.stopPropagation()}
              ...
              <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} className="relative w-full max-w-sm floating-card p-5 max-h-[90vh] flex flex-col">
                ...
                <div className="overflow-y-auto flex-1 space-y-4 pr-1">
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      {([{ key: 'work' as const, label: 'Focus' }, { key: 'shortBreak' as const, label: 'Short' }, { key: 'longBreak' as const, label: 'Long' }] as { key: keyof TimerSettings; label: string }[]).map(({ key, label }) => (
                        <div key={key} className="flex flex-col items-center gap-1 bg-white/5 border border-white/10 rounded-xl p-2">
                          ...
                          <input
                            type="number" min="1" max="120" value={timerSettings[key]}
                    ...
                    <button type="button" onClick={() => { setTimerSettings(DEFAULT_TIMER_SETTINGS); setIsActive(false); setTimeLeft(DEFAULT_TIMER_SETTINGS[mode] * 60); }}
                      className="w-full mt-2 py-1.5 bg-white/5 hover:bg-white/10 border border-white/10 rounded-lg text-[10px] text-white/50 hover:text-white">
                      Reset to Defaults
                    </button>
                  </div>
