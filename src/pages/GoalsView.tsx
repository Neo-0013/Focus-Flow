...
export function GoalsView({ goals, tasks, fetchGoals, workspace, aiConfig, showToast }: GoalsProps) {
  ...
  const handleAddGoal = async () => {
    ...
    try {
      ...
      setNewGoalTitle('');
      setNewGoalTarget('1');
      showToast?.('Goal Added! 🎯', `"${newGoalTitle}" added to your ${goalView} goals.`, 'success');
    } catch (err: any) {
      showToast?.('Error', err?.response?.data?.error || 'Failed to create goal. Is the server running?', 'error');
    }
  };
