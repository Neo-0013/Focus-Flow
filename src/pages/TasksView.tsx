...
export function TasksView({
  ...
  archiveCompletedTasks, fetchGoals, fetchTasks, workspace, aiConfig, showToast
  ...
  const handleGenerateRoadmap = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE}/neo/completion`, {
        method: 'POST',
        headers: {
    ...
    <motion.div key="tasks" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="max-w-4xl mx-auto space-y-8">
      ...
      <div className="space-y-4 pb-20">
        ...
        {filteredTasks.map(task => {
          ...
          return (
            <motion.div layout initial={{opacity:0, scale:0.95}} animate={{opacity:1, scale:1}} exit={{opacity:0, scale:0.95}} key={task.id} className="bg-panel/40 backdrop-blur-2xl border border-white/5 shadow-[inset_0_0_20px_rgba(255,255,255,0.02)] rounded-[32px] overflow-hidden transition-all duration-300 hover:bg-white/[0.03] hover:border-white/10 hover:shadow-[0_8px_32px_rgba(0,0,0,0.4)]">
              <div className="p-6 flex flex-col gap-4">
                <div className="flex items-start gap-6">
                  <button 
                    onClick={() => toggleTask(task.id)} 
                    className={cn("transition-all hover:scale-110 mt-1", task.completed ? "text-emerald-500 drop-shadow-[0_0_8px_rgba(16,185,129,0.5)]" : "text-white/20 hover:text-white")}
                  >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      {task.goalId && goals.find(g => g.id === task.goalId) && (
                         <span className="px-2 py-0.5 rounded-[4px] text-[9px] uppercase font-bold text-blue-900 bg-blue-400 border border-blue-400/50 shadow-[0_0_10px_rgba(96,165,250,0.2)]">
                    ...
                    <div className="flex items-center gap-3 mt-4">
                      ...
                      <div className="flex-1 h-1.5 bg-white/[0.03] rounded-full overflow-hidden shadow-inner border border-white-[0.01]">
                        <motion.div 
                          initial={{ width: 0 }} 
                          animate={{ width: `${progress}%` }} 
                          className="h-full bg-gradient-to-r from-emerald-600 to-emerald-400 shadow-[0_0_20px_rgba(16,185,129,0.8)] rounded-full relative" 
                        >
                  ...
                  <button onClick={() => deleteTask(task.id)} className="p-3 text-white/10 hover:text-red-500 hover:bg-red-500/10 rounded-xl transition-all">
                    <Trash2 className="w-5 h-5" />
                  </button>
                </div>
                
                {/* Inline Editing for Date & Time */}
                <div className="flex items-center gap-6 ml-[52px] pt-3 mt-1 border-t border-white/5">
                  <div className="flex items-center gap-2 relative group/date cursor-pointer">
                    <Calendar className={cn("w-3.5 h-3.5 transition-colors", task.dueDate ? "text-amber-400/80" : "text-white/20")} />
                    <input 
                      type="date"
                      defaultValue={task.dueDate || ''}
                      onChange={async (e) => {
                        await axios.patch(`${API_BASE}/tasks/${task.id}`, { dueDate: e.target.value || null });
                      }}
                      className={cn(
                        "bg-transparent text-xs font-bold focus:outline-none cursor-pointer appearance-none transition-colors",
                        task.dueDate ? "text-amber-400/80 hover:text-amber-400" : "text-white/20 hover:text-white/40"
                      )}
                    />
                  </div>
                  
                  <div className="flex items-center gap-2 relative group/time cursor-pointer">
                    <Clock className={cn("w-3.5 h-3.5 transition-colors", task.timeSlot ? "text-cyan-400/80" : "text-white/20")} />
                    <input 
                      type="time"
                      defaultValue={task.timeSlot || ''}
                      onChange={async (e) => {
                        await axios.patch(`${API_BASE}/tasks/${task.id}`, { timeSlot: e.target.value || null });
                      }}
                      className={cn(
                        "bg-transparent text-xs font-bold focus:outline-none cursor-pointer appearance-none transition-colors",
                        task.timeSlot ? "text-cyan-400/80 hover:text-cyan-400" : "text-white/20 hover:text-white/40"
                      )}
                    />
                  </div>
                </div>
              ...
              <AnimatePresence>
                {isExpanded && (
                  <motion.div 
                    ...
                    className="bg-black/30 backdrop-blur-xl border-t border-white/5 p-6 rounded-b-[32px] space-y-4 shadow-inner"
                  >
                    {(task.subTasks || []).map(st => (
                      <div key={st.id} className="flex items-center gap-4 group/sub hover:bg-white/[0.02] p-2 -mx-2 rounded-xl transition-colors">
                        <button 
                          onClick={async () => {
                          ...
                          className={cn(
                            "w-5 h-5 rounded-lg border flex items-center justify-center transition-all hover:scale-110 shadow-sm", 
                            st.completed ? "bg-emerald-500 border-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.4)]" : "border-white/10 hover:border-white/30 hover:bg-white/5"
                          )}
                        >
                          {st.completed && <Check className="w-3.5 h-3.5 text-black" />}
                        </button>
                        <span className={cn("flex-1 text-sm transition-all", st.completed ? "text-white/20 line-through" : "text-white/70 font-medium")}>
                          {st.text}
                        </span>
                      </div>
                    ))}
                    <div className="relative pt-2">
                      <Plus className="absolute left-4 top-[60%] -translate-y-1/2 w-4 h-4 text-white/20" />
                      <input 
                        placeholder="Add sub-task..." 
                        className="w-full bg-white/[0.03] border border-white/10 rounded-2xl py-3.5 pl-12 pr-4 text-xs font-medium focus:outline-none focus:border-white/20 focus:bg-white/[0.05] transition-all shadow-inner hover:bg-white/[0.04]" 
                        onKeyDown={async (e) => {
